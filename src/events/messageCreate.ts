    ...
    PREFIX.forEach(async (botPrefix: string) => {
      ...
      const command: MessageCommand | undefined =
        ...
        client.messageCommands?.get(
          client.messageCommands_Aliases?.get(commandName) ?? "",
        );
      if (!command) return;
      const args: Array<string> = message.content
        ...
        .trim()
        .split(" ");
      const processedCommandOptionsChecker: boolean =
        await commandOptionsChecker(client, message, command, "MessageCommand");

      if (command?.allowInDms)
        return processedCommandOptionsChecker

...
export const SlashManager = async (
  ...
  const allSlashCommandsFiles = fileReader(
    `${rootPath}/interactions/slashCommands`,
  );
  const allContextMenusFiles = fileReader(
  ...
  if (allSlashCommandsFiles.length > 0) {
    for (const slashCommandFile of allSlashCommandsFiles) {
      const slashCommand: SlashCommand | undefined = (
        await import(`file:///${slashCommandFile}`)
      )?.Slash;
  ...
  if (allContextMenusFiles.length > 0) {
    for (const contextMenuFile of allContextMenusFiles) {
      const contextMenu: ContextMenu | undefined = (
        await import(`file:///${contextMenuFile}`)
      )?.Context;
  ...
  try {
    ...
    for (const guildObject of Object.entries(guildCommandsObject)) {
      await rest.put(
        Routes.applicationGuildCommands(client.application.id, guildObject[0]),
        {
          body: guildObject[1],
        },
      );

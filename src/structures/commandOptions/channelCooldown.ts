...
export const channelCooldownFN = async (
  ...
  if (
    Math.floor(currentTime - storedTime) >= command.channelCooldown ||
    !storedTime
  ) {
    ...
    message.channel?.send({
      embeds: [
        new EmbedBuilder()
          ...
          .setDescription(
            `You are currently at cooldown. Please try again in <t:${Math.floor(Math.floor(storedTime + command.channelCooldown) / 1000)}:R>.`,
          ),
      ],
    });
    return false;

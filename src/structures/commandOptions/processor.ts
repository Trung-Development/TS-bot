...
export default async (
  ...
  const allClientPermissions: boolean = allClientPermissionsFN(
    client,
    message,
    command,
  );
  const anyClientPermissions: boolean = anyClientPermissionsFN(
    client,
    message,
    command,
  );
  const allUserPermissions: boolean = allUserPermissionsFN(
    client,
    message,
    command,
  );
  const anyUserPermissions: boolean = anyUserPermissionsFN(
    client,
    message,
    command,
  );

  const channelCooldown: boolean = await channelCooldownFN(
    client,
    message,
    command,
    interactionType,
  );
  const globalCooldown: boolean = await globalCooldownFN(
    client,
    message,
    command,
    interactionType,
  );
  const guildCooldown: boolean = await guildCooldownFN(
    client,
    message,
    command,
    interactionType,
  ...
  const finalCorrection: Array<boolean> = [

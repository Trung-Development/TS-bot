...
export const globalCooldownFN = async (
  ...
  if (
    Math.floor(currentTime - storedTime) >= command.globalCooldown ||
    !storedTime
  ) {
    ...
    message.channel?.send({
      embeds: [
        new EmbedBuilder()
          ...
          .setDescription(
            `You are currently at cooldown. Please try again in <t:${Math.floor(Math.floor(storedTime + command.globalCooldown) / 1000)}:R>.`,
          ),
      ],
    });
    return false;

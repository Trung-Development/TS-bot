...
export const guildCooldownFN = async (
  client: DiscordClient,
  message: Message | Interaction<"cached">,
  command: AnyCommand,
  interactionType: InteractionTypeOptions,
): Promise<boolean> => {
  if (!command.guildCooldown || isNaN(command.guildCooldown) || !message.guild)
    return true;
  ...
  if (
    Math.floor(currentTime - storedTime) >= command.guildCooldown ||
    !storedTime
  ) {
    ...
    message.channel?.send({
      embeds: [
        new EmbedBuilder()
          ...
          .setDescription(
            `You are currently at cooldown. Please try again in <t:${Math.floor(Math.floor(storedTime + command.guildCooldown) / 1000)}:R>.`,
          ),
      ],
    });
    return false;
